...
命令会自动打开授权页面。登录皮可AI并确认授权后，页面会跳转到本机回调页，脚本随后自动创建并保存 API Key。命令会一直等待授权结果，不要让用户在对话中发送密钥，也不要要求用户另行回复“已完成”。浏览器无法打开时，使用命令显示的链接和验证码；`auth login --no-browser` 可保留纯手动流程。
